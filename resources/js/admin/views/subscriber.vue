                                    ...
                                    <div id="order-listing_filter" class="dataTables_filter">
                                        <label class="w-100">
                                            <input type="text" class="form-control w-100" placeholder="Search By Title..."
                                                name="searchVal" v-model="searchVal" @keyup="searchSubscriber()"
                                    ...
                                    <table id="order-listing" class="table dataTable no-footer"
                                        ...
                                        <tbody>
                                            <tr v-for="subscriber in subscribers.data" :key="subscriber.id">
                                                ...
                                                <td> {{ subscriber.email.length > 20 ? subscriber.email.slice(0, 20) + '...' :
                                                subscriber.email }} </td>
                                                <td>
                                                    <button type="button" data-bs-toggle="modal"
                                                        data-bs-target="#showDataModal" @click="showSubscriber(subscriber)"
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-5">
                                    <div class="dataTables_info" id="order-listing_info" role="status"
                                        aria-live="polite">Showing {{ ( (subscribers.total + 1) - subscribers.from) }} to {{ (
                                        (subscribers.total + 1) - subscribers.to) }} of {{ subscribers.total }} entries</div>
                                </div>
                                <div class="col-sm-12 col-md-7">
                                    <div class="dataTables_paginate paging_simple_numbers">
                                        <!-- pagination Component -->
                                        <!-- <Pagination :data="subscribers" @pagination-change-page="getSubscribers" :limit="1"/> -->
                                        <pagination :data="subscribers" @pagination-change-page="getSubscribers" :limit="2">
                                            <template #prev-nav>
                                                <span>&lt; </span>
                                            </template>
                                            <template #next-nav>
                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="ModalLabel">Show Subscriber Data</h5>
...
export default {
    ...
    methods: {
        ...
        getSubscribers(page = 1) {
            axios.get('/api/admin/subscriber?page=' + page)
                .then(
                    response => {
        ...
        showSubscriber(subscriber) {
            axios.get('/api/admin/subscriber/' + subscriber.id)
                .then(
                    response => {
                        // console.log(response.data);
        ...
        deletePost(subscriber) {
            this.$swal({
                text: 'Are you sure delete subscriber ?',
                icon: 'warning',
                ...
                if (result.value) {
                    // do delete response
                    axios.delete("/api/admin/subscriber/" + subscriber.id)
                        .then(
                            response => {  // if there success request

                                if (response.data.status == "success") {

                                    this.getSubscribers(); // reload getSubscribers()
                                    this.subscriber = {}      // empty subscriber var
        ...
        searchSubscriber() {
            ...
            axios.post("/api/admin/subscriber/search", formData, config)
                .then(
                    response => {  // if there success request
            ...
            if (this.multiAction.action == '') {
                ...
                this.$swal({
                    ...
                    if (result.value) {
                        ...
                        // Send request with axios
                        axios.post("/api/admin/subscriber/multiAction", formData, config)
                        .then(
                            response => {  // if there success request
